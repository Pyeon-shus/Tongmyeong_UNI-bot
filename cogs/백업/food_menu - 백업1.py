...
class FoodMenu(commands.Cog):
        ...
        try:
            ...
            if table:
                ...
                for menu, title in [(yangsik, "#양식"), (myeonryu, "#면류"), (bunsik, "#분식"), 
                                    (teukjeongsik, "#특정식"), (ddukbaegi, "#뚝배기"), (ilpum, "#일품")]:
                    if len(menu) > 0:
                        result = '\n'.join(['\n'.join(row) for row in menu])
                        embed.add_field(name=title, value=f"{result}\n\n", inline=False)
    ...
    async def dormitory_menu(self, interaction: discord.Interaction):
        ...
        try:
            ...
            embed.add_field(name="#조식", value=breakfast_menu_str if breakfast_menu_str else "오늘 조식 메뉴가 없습니다.", inline=False)
            embed.add_field(name="\n", value=f"\n", inline=False)
