...
class AcademicSchedule(commands.Cog):
    ...
    async def academic_schedule(self, interaction: discord.Interaction, 달월: app_commands.Choice[int] = None):
        ...
        try:
            if((datetime.datetime.now().month<3)&(selected_month<3)):
                url = f'https://www.tu.ac.kr/tuhome/scheduleTable.do?schType=U&siteId=tuhome&schYear={datetime.datetime.now().year-1}&schKeyword='
