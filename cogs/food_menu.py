...
class FoodMenu(commands.Cog):
    ...
    @app_commands.describe(내일="내일의 학식을 보여줍니다 (선택하지 않으면 오늘의 학식을 보여줍니다).")
    @app_commands.choices(내일=[
        app_commands.Choice(name="내일", value=1)  # 내일을 선택하면 값이 1
    ])
    async def show_food_menu(self, interaction: discord.Interaction, 내일: app_commands.Choice[int] = None):
        # 기본값 처리: 선택하지 않으면 오늘(0), 선택하면 내일(1)
        kst = pytz.timezone('Asia/Seoul')
        current_time = datetime.datetime.now(kst)

        if 내일 is not None and 내일.value == 1:
            # 내일의 날짜 계산
            cal_date = "Date="+(current_time + datetime.timedelta(days=1)).strftime("%Y-%m-%d")
            titled = "내일의 학식"
        else:
            # 오늘의 날짜
            cal_date = ""
        ...
        try:
            url = f'https://www.tu.ac.kr/tuhome/diet.do?sch{cal_date}'
            ...
            if table:
                ...
                embed = discord.Embed(title=f":fork_and_knife:{titled}:fork_and_knife:", description="",timestamp=datetime.datetime.now(pytz.timezone('UTC')), color=0x00b992)
                ...
                printf=f'({interaction.user.name}님의 입력)_{titled} 정상 출력됨'
                print(printf)
                append_to_log(printf, LOG_FILE)
                await interaction.followup.send(embed=embed)
            else:
                embed = discord.Embed(title=f":fork_and_knife:{titled}:fork_and_knife:", description="",timestamp=datetime.datetime.now(pytz.timezone('UTC')), color=0x96C81E)
                embed.set_thumbnail(url="https://cdn-icons-png.flaticon.com/512/4474/4474873.png")
                embed.add_field(name="\n", value=f"\n", inline=False)
                ...
                printf=f'({interaction.user.name}님의 입력)_{titled} 정상 출력됨 (메뉴 없음)'
                print(printf)
                append_to_log(printf, LOG_FILE)
                await interaction.followup.send(embed=embed)
        except Exception as e:
            error_message = f"메뉴를 가져오는 중 오류가 발생했습니다:\n```\n{type(e).__name__}: {str(e)}\n```"
            printf=f'({interaction.user.name}님의 입력)_{titled} 오류 출력됨\n{error_message}'
            print(printf)
            append_to_log(printf, LOG_FILE)
